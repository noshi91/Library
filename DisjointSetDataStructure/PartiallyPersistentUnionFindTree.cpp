...
class PartiallyPersistentUnionFindTree
{
  ...
  public:
    PartiallyPersistentUnionFindTree(const uint32 size)
        : tree(size, {1, std::numeric_limits<uint32>::max()}), siz(size, std::vector<std::pair<uint32, uint32>>(1, {0, 1})), global_count(0) {}
    ...
    {
        x = find(ver, x);
        return (std::lower_bound(siz[x].begin(), siz[x].end(), std::pair<uint32, uint32>(ver + 1, 0)) - 1)->second;
    }
    uint32 count(void)
    {
        return global_count;
